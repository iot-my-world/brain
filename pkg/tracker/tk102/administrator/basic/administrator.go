...
import (
	...
	"github.com/iot-my-world/brain/pkg/tracker/tk102/action"
	administrator2 "github.com/iot-my-world/brain/pkg/tracker/tk102/administrator"
	"github.com/iot-my-world/brain/pkg/tracker/tk102/administrator/exception"
	recordHandler2 "github.com/iot-my-world/brain/pkg/tracker/tk102/reading/recordHandler"
	"github.com/iot-my-world/brain/pkg/tracker/tk102/recordHandler"
	"github.com/iot-my-world/brain/pkg/tracker/tk102/validator"
)

type administrator struct {
	tk102RecordHandler   recordHandler.RecordHandler
	companyRecordHandler *companyRecordHandler.RecordHandler
	clientRecordHandler  *clientRecordHandler.RecordHandler
	partyAdministrator   partyAdministrator.Administrator
	readingRecordHandler recordHandler2.RecordHandler
...
func (a *administrator) ValidateChangeOwnershipAndAssignmentRequest(request *administrator2.ChangeOwnershipAndAssignmentRequest) error {
	...
	if request.Claims == nil {
		...
		tk102ValidateResponse, err := a.tk102DeviceValidator.Validate(&validator.ValidateRequest{
...
*/
func (a *administrator) ChangeOwnershipAndAssignment(request *administrator2.ChangeOwnershipAndAssignmentRequest) (*administrator2.ChangeOwnershipAndAssignmentResponse, error) {
	if err := a.ValidateChangeOwnershipAndAssignmentRequest(request); err != nil {
		return nil, err
	}

	// 1. retrieve the tk102 device
	tk102RetrieveResponse, err := a.tk102RecordHandler.Retrieve(&recordHandler.RetrieveRequest{
		Claims:     request.Claims,
	...
	// 2. collect readings for the device
	readingCollectResponse, err := a.readingRecordHandler.Collect(&recordHandler2.CollectRequest{
		Claims: request.Claims,
	...
	if err != nil {
		return nil, exception.ReadingCollection{Reasons: []string{err.Error()}}
	}
	...
	tk102UpdateResponse, err := a.tk102RecordHandler.Update(&recordHandler.UpdateRequest{
		Claims:     request.Claims,
		Identifier: id.Identifier{Id: request.TK102.Id},
	...
	for readingIdx := range readingCollectResponse.Records {
		...
		if _, err := a.readingRecordHandler.Update(&recordHandler2.UpdateRequest{
			Claims:     request.Claims,
			...
			return nil, exception.ReadingUpdate{Reasons: []string{err.Error()}}
		}
	}

	return &administrator2.ChangeOwnershipAndAssignmentResponse{TK102: tk102UpdateResponse.TK102}, nil
}
...
func (a *administrator) ValidateCreateRequest(request *administrator2.CreateRequest) error {
	...
	if request.Claims == nil {
		...
		tk102DeviceValidateResponse, err := a.tk102DeviceValidator.Validate(&validator.ValidateRequest{
			Claims: request.Claims,
			TK102:  request.TK102,
...
func (a *administrator) Create(request *administrator2.CreateRequest) (*administrator2.CreateResponse, error) {
	...
	// Create the device
	createResponse, err := a.tk102RecordHandler.Create(&recordHandler.CreateRequest{
		TK102: request.TK102,
	})
	if err != nil {
	...
	return &administrator2.CreateResponse{TK102: createResponse.TK102}, nil
}
